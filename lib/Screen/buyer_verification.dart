...
class BuyerVerificaion extends StatefulWidget {
  String countrycode;
  String mobile;

  BuyerVerificaion({Key key, this.countrycode, this.mobile}) : super(key: key);

  @override
  State<BuyerVerificaion> createState() => _BuyerVerificaionState();
}

class _BuyerVerificaionState extends State<BuyerVerificaion> {
  ...
  final BuyerAuthController buyerAuth = Get.put(BuyerAuthController());
  @override
  void initState() {
    super.initState();
  ...
  void _signInWithPhoneNumber(String otp) async {
    ...
    if (await checkInternet()) {
      try {
        ...
        final User user =
            (await buyerAuth.firebaseAuth.signInWithCredential(credential))
                .user;
        final User currentUser = buyerAuth.firebaseAuth.currentUser;
        assert(user.uid == currentUser.uid);
        print("verify id is ${currentUser.uid}");
        buyerAuth.isUserExists();
